...
class SlyeBase extends EventEmitter {
  constructor(props, uuid) {
    ...
    this.props	= props || {};
    this.uuid	= uuid;
    if (!this.props._randoms_) { this.props._randoms_ = {}; }
  ...
  random(scope) {
    ...
    if (this.props._randoms_[scope].length > this._randomOffset_[scope]) { return this.props._randoms_[scope][this._randomOffset_[scope]]; }
  ...
  generateUUID(scope) {
    ...
    if (this.props._uuids_[scope].length > this._uuidOffset_[scope]) { return this.props._uuids_[scope][this._uuidOffset_[scope]]; }
  ...
  codeScope(name, func) {
    const prev = this.__scopeName__;
    this.__scopeName__	= name;
    const random	= () => this.random(name);
    const generateUUID	= () => this.generateUUID(name);
    func(random, generateUUID);
    this.__scopeName__	= prev;
  }

  _return_(r) {
    if (r.uuid) { r.uuid	= this.generateUUID(); }
    return r;
  }

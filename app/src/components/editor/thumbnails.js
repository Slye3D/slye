...
class Thumbnail extends Component{
	...
	render(){
		let id		= Fileformat.Path.getPath().indexOf(this.props.uuid)
		let className = ['thumbnail', id == this.state.selectedStep ? 'selected' : ''].join(' ')
		return (
			<div className={className} onClick={this.onClick(id)} ref={(el) => Renderer.Thumbnails.setDom(this.props.uuid, el)} x={this.props.uuid}>
				<div className='num'>
					{id + 1}
				</div>
...
class Thumbnails extends Component{
	...
	toggleThumbnails = () => {
		let newState	= !this.state.isThumbnailsOpen
		if(newState)
	...
	render(){
		...
		Fileformat.Path.getPath().map((stepUUID, id) => {
			if(!thumbnailsCache[stepUUID])
				thumbnailsCache[stepUUID] = <Thumbnail key={stepUUID} uuid={stepUUID} />
			thumbs.push(thumbnailsCache[stepUUID])
		})
		let Canvas	= Renderer.Thumbnails.getThumbnailCanvas()
		return (
			<Paper className={this.state.isThumbnailsOpen ? 'open' : 'close'} id="thumbnails-wrapper"  zDepth={3}  style={{

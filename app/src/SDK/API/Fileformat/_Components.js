...
function removeComponent(componentUUID){
	if(!presentation.components[componentUUID])
		return false
	// Remove component from owner step
	let stepUUID = presentation.__cache__.c2s[componentUUID]
	if(stepUUID)
		removeComponentFromStep(stepUUID, componentUUID)
...
function setComponentProps(uuid, props){
	if(!presentation.components[uuid])
		return
	for(let key in props)
		presentation.components[uuid][key] = props[key]
	return true
}

...
export default {
	...
	createRenderer(name, w, h, far = 1000, near = 0.1, fov = 75){
		if(global.__renderers__[name])
			return
		let renderer = new THREE.WebGLRenderer()
		renderer.setSize(w, h)
		renderer.setPixelRatio(window.devicePixelRatio)

		let camera	= new THREE.PerspectiveCamera(fov, w / h, near, far)
	...
	render(name){
		if(!global.__renderers__[name])
			return
		let {renderer, camera} = global.__renderers__[name]
	...
	intersects(name, event, element, width){
		...
		let intersects = raycaster.intersectObjects(global.__scene__.children)

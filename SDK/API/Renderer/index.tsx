...
function requestAnimationFrame() {
  ...
  if (global.__isInPlayMode__) {
    ...
    const stepUUIDs	= Steps.getSteps();
    const a 			= global.__renderCallNum__ % 4;
    const current		= stepUUIDs.indexOf(Navigation.currentUUID);
...
function hardRender(stepUUID) {
  ...
  group.uuid	= stepUUID;
  let tmp;
  const components = Steps.getStep(stepUUID).components;
  for (const uuid of components) {
    const obj = Components.getComponentObject(uuid).__three__;
    if (!obj) { return; }
